...
#if APP_FACE_RECOG_BACKEND == APP_AI_BACKEND_TFLM_INT8
...
static const char *TAG = "face_recog_tflm_int8";
...
static constexpr uint32_t DB_VERSION = 1U;

struct __attribute__((packed)) tflm_int8_db_header_t {
    char magic[8];
    uint32_t version;
...
struct __attribute__((packed)) tflm_int8_db_entry_prefix_t {
    ...
    bool *used_eye_alignment)
{
    ...
    auto write_real = [&](float value) {
        ...
        out[index++] = static_cast<int8_t>(q);
    };
    ...
    for (int dy = 0; dy < APP_TFLM_MOBILEFACENET_INPUT_SIZE; ++dy) {
        ...
        for (int dx = 0; dx < APP_TFLM_MOBILEFACENET_INPUT_SIZE; ++dx) {
            ...
            /*
             * Inverse affine sampling is identical to the FP32 backend.
             * Only the final normalized value is quantized into the model's
             * INT8 input tensor.
             */
            ...
            write_real(normalize_mobilefacenet_pixel(r));
            write_real(normalize_mobilefacenet_pixel(g));
            write_real(normalize_mobilefacenet_pixel(b));
        }
    }
    ...
    embedding_timing_t *timing)
{
    ...
    *timing = {};
    ...
    float norm_sq = 0.0f;
    int output_q_min = 127;
    int output_q_max = -128;
    unsigned output_nonzero_from_zp = 0;

    for (size_t i = 0;
         i < APP_TFLM_MOBILEFACENET_EMBEDDING_DIM;
         ++i) {
        const int q = static_cast<int>(output->data.int8[i]);
        output_q_min = std::min(output_q_min, q);
        output_q_max = std::max(output_q_max, q);
        if (q != output->params.zero_point) {
            output_nonzero_from_zp++;
        }
    ...
    if (norm <= 1e-12f || !isfinite(norm)) {
        ESP_LOGE(
            TAG,
            "INT8 embedding collapsed: q_min=%d q_max=%d nonzero_from_zp=%u "
            "zero_point=%d scale=%.9f norm_sq=%.9g",
            output_q_min,
            output_q_max,
            output_nonzero_from_zp,
            output->params.zero_point,
            (double)output->params.scale,
            (double)norm_sq);
        return ESP_FAIL;
    }
    ...
    uint64_t *fingerprint)
{
    ...
    *fingerprint = fnv1a_update_cstring(
        *fingerprint,
        "TFLM_INT8_MOBILEFACENET_ALIGNMENT_V1");
    *fingerprint = fnv1a_update_cstring(
        *fingerprint,
     ...
     */
    (void)hash_file(APP_TFLM_INT8_FACE_DETECT_MODEL_PATH, fingerprint);
#endif
    ...
    ESP_LOGI(
        TAG,
        "TFLM-INT8+ESP-NN enrollment scan: people=%u images=%u fingerprint=%016llx",
...
{
    FILE *file = fopen(APP_TFLM_INT8_RECOG_DATABASE_PATH, "wb");
    if (!file) {
        return ESP_FAIL;
    }

    tflm_int8_db_header_t header = {};
    ...
    for (const embedding_entry_t &entry : s_entries) {
        tflm_int8_db_entry_prefix_t prefix = {};
        snprintf(
            prefix.name,
            sizeof(prefix.name),
            "%s",
...
static bool load_database(
    ...
    size_t expected_dim)
{
    FILE *file = fopen(APP_TFLM_INT8_RECOG_DATABASE_PATH, "rb");
    if (!file) {
        return false;
    }
    ...
    uint64_t fingerprint)
{
    ...
    unsigned rejected = 0;
    unsigned rejected_no_face = 0;
    unsigned rejected_embedding = 0;
    unsigned rejected_read = 0;
    ...
    for (const enrollment_image_t &source : images) {
        ...
        if (read_rgb888_file(source.full_path.c_str(), &rgb) != ESP_OK) {
            ESP_LOGW(
                TAG,
                "Enrollment rejected: person=%s file=%s reason=read_failed",
        ...
        /*
         * Keep the same small yield used by the FP32 enrollment path. It does
         * not affect live recognition and lets CPU1 service housekeeping while
        ...
        ESP_LOGI(
            TAG,
            "[AI-BENCH] backend=TFLM-INT8+ESP-NN model=MobileFaceNet "
    ...
    if (s_entries.empty()) {
        /*
         * Diagnostic-safe boot:
         * Do not enter an ESP_ERROR_CHECK reboot loop when enrollment fails.
         * Keep the recognizer initialized with an empty DB so the live camera
         * and INT8 detector can run and expose their benchmark/score logs.
         * Recognition simply returns "unknown" while s_entries is empty.
         *
         * This does not alter DL or FP32 behavior and is not intended as the
         * final benchmark state; it exists to identify the INT8 enrollment
         * failure without hiding it behind repeated resets.
         */
        ESP_LOGE(
            TAG,
            "TFLM-INT8+ESP-NN database rebuild produced zero embeddings: "
            "images=%u rejected=%u read_fail=%u no_face=%u embedding_fail=%u; "
            "continuing with EMPTY INT8 DB for diagnostics",
            (unsigned)images.size(),
            rejected,
            rejected_read,
            rejected_no_face,
    ...
    if (save_ret != ESP_OK) {
        ESP_LOGE(
            TAG,
            "Could not save %s: %s",
            APP_TFLM_INT8_RECOG_DATABASE_PATH,
    ...
    ESP_LOGI(
        TAG,
        "TFLM-INT8+ESP-NN database rebuilt: entries=%u rejected=%u dim=%u path=%s",
...
extern "C" esp_err_t tflm_int8_face_recognition_init(void)
{
    const esp_err_t init_ret = s_runner.init(
        APP_TFLM_INT8_FACE_RECOG_MODEL_PATH,
    ...
    if (!exact_recognition_input(input) ||
        ...
        ESP_LOGE(
            ...
            "input=[1,112,112,3] INT8, output0=[1,192] INT8");
        return ESP_ERR_NOT_SUPPORTED;
    }
    ...
    ESP_LOGI(
        TAG,
        "TFLM-INT8+ESP-NN MobileFaceNet ready: model=%s dim=%u arena_used=%u "
        "input_scale=%.9f input_zero=%d output_scale=%.9f output_zero=%d",
        APP_TFLM_INT8_FACE_RECOG_MODEL_PATH,
        (unsigned)s_embedding_dim,
    ...
    if (load_database(fingerprint, s_embedding_dim)) {
        ESP_LOGI(
            TAG,
            "TFLM-INT8+ESP-NN database loaded: entries=%u dim=%u path=%s",
            (unsigned)s_entries.size(),
            (unsigned)s_embedding_dim,
            APP_TFLM_INT8_RECOG_DATABASE_PATH);
        return ESP_OK;
...
extern "C" int tflm_int8_face_recognition_get_count(void)
{
    return static_cast<int>(s_entries.size());
}

extern "C" esp_err_t tflm_int8_face_recognition_recognize(
    uint8_t *camera_buf,
    uint32_t width,
    uint32_t height,
    const face_box_t *box,
    ...
    float *out_score)
{
    ...
    *out_score = best_score;
    ...
    ESP_LOGI(
        TAG,
        "TFLM-INT8+ESP-NN recognition: person=%s similarity=%.3f threshold=%.3f",
        out_name,
        (double)best_score,
