#!/usr/bin/env python3
"""
v3: Quantize existing FLOAT32 TFLite FlatBuffers directly to full INT8.
...
def quantize_one(
    ...
    print_model_summary(f"INT8 {dst.name}", quantized)
    describe_operator_codes(quantized, f"INT8 {dst.name}")
    verify_strict_int8(dst, quantized)

    # Only write after all validation passes.
    dst.write_bytes(quantized)
    print(f"\n[OK] Strict INT8 model written: {dst}")
