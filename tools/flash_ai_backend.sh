#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
IDF_PROJECT="$PROJECT_ROOT/src_p4"

MODE="${1:-}"
PORT="${2:-}"

case "$MODE" in
    dl|espdl|esp-dl)
        CANONICAL="espdl"
        LABEL="ESP-DL S8"
        ;;
    fp32|tflm|tflm-fp32)
        CANONICAL="tflm-fp32"
        LABEL="TFLM-FP32"
        ;;
    nn|int8|esp-nn|tflm-int8|tflm-int8-espnn)
        CANONICAL="tflm-int8"
        LABEL="TFLM-INT8 + ESP-NN"
        ;;
    *)
        echo "Usage: $0 {dl|fp32|nn} [PORT]"
        echo "Examples:"
        echo "  $0 dl"
        echo "  $0 fp32 /dev/ttyACM0"
        echo "  $0 nn"
        exit 2
        ;;
esac

if [[ ! -f "$IDF_PROJECT/CMakeLists.txt" ]]; then
    echo "[ERROR] ESP-IDF project not found:"
    echo "  $IDF_PROJECT/CMakeLists.txt"
    exit 1
fi

"$SCRIPT_DIR/select_ai_backend.sh" "$CANONICAL"

# The device reads the models from /sdcard. If the same card is mounted on the
# host, validate the files before flashing. If it is already inside the board,
# this host-side check is skipped and the firmware validates the tensor contract
# at boot.
HOST_SD="${APP_HOST_SD_ROOT:-/media/${USER}/SDCARD}"

check_model() {
    local path="$1"
    local expected_sha="$2"

    if [[ ! -f "$path" ]]; then
        echo "[ERROR] Required model missing: $path"
        return 1
    fi

    local actual
    actual="$(sha256sum "$path" | awk '{print $1}')"
    echo "  $(basename "$path")  $actual"

    if [[ -n "$expected_sha" && "$actual" != "$expected_sha" ]]; then
        echo "[ERROR] Model hash does not match the validated model."
        echo "        expected: $expected_sha"
        echo "        actual  : $actual"
        return 1
    fi
}

if [[ -d "$HOST_SD/models" ]]; then
    echo
    echo "========== SD MODEL CHECK =========="
    case "$CANONICAL" in
        tflm-fp32)
            check_model "$HOST_SD/models/FDET32.TFL" \
                "7493838b71715d96ef235ce6e3fc41f6b8feefb54214ec4c0bbf459fd6778bb7"
            check_model "$HOST_SD/models/FREC.TFL" \
                "be4bc7cfc53f7bc336d0f28b1ab92535f618c913a422b683210750f6b5354854"
            ;;
        tflm-int8)
            check_model "$HOST_SD/models/FDET8.TFL" \
                "d17bbb42ac96992b2e069d18c13013f9dbe9fd89c40ef5a3a6a05fb2b221497f"
            check_model "$HOST_SD/models/FREC8.TFL" \
                "83a8e78a434580bec16ae30a4c336d8f39a249a46d55521250ed94b5dcc260c3"
            ;;
        espdl)
            echo "ESP-DL models are supplied by the Espressif components."
            ;;
    esac
else
    echo
    echo "[INFO] Host SD card is not mounted at $HOST_SD."
    echo "       Continuing; the firmware will read /sdcard on the ESP32-P4."
fi

# Preserve the established common power/scheduling policy for every backend.
#
# Architecture-aligned source layout:
#   Vision owns DET -> REC and its inference guard.
#   Application Logic wires the Vision boost hook to the neutral System Adapter port.
#   The System Adapter port delegates to the concrete CPU-power adapter.
echo
echo "========== COMMON POLICY CHECK =========="
grep -Eq '^#define[[:space:]]+APP_CPU_MAX_FREQ_MHZ[[:space:]]+360([[:space:]]|$)' \
    "$PROJECT_ROOT/src_p4/main/include/config/app_config.h"
grep -Eq '^#define[[:space:]]+APP_CPU_ACTIVE_FREQ_MHZ[[:space:]]+180([[:space:]]|$)' \
    "$PROJECT_ROOT/src_p4/main/include/config/app_config.h"

# DET -> REC serialization remains inside Face Recognition Service.
grep -q 'vision_ai_inference_guard_lock' \
    "$PROJECT_ROOT/src_p4/main/services/vision/ai_worker_pipeline.c"

# Vision requests boost through its hook; Application Logic wires that hook to
# the neutral System Adapter port; the adapter delegates to cpu_power.
grep -q 's_hooks\.acquire_face_boost' \
    "$PROJECT_ROOT/src_p4/main/services/vision/ai_worker_pipeline.c"
grep -q '\.acquire_face_boost[[:space:]]*=[[:space:]]*system_cpu_face_boost_begin' \
    "$PROJECT_ROOT/src_p4/main/app/camera/camera_session.c"
grep -q 'return cpu_power_face_boost_begin();' \
    "$PROJECT_ROOT/src_p4/main/platform/system/system_adapters_port.c"

# DET and REC remain in the Face Recognition Service pipeline.
grep -q 'face_detect_run_rgb' \
    "$PROJECT_ROOT/src_p4/main/services/vision/ai_worker_pipeline.c"
grep -q 'face_recognition_recognize' \
    "$PROJECT_ROOT/src_p4/main/services/vision/ai_worker_pipeline.c"

echo "  baseline CPU       : 180 MHz"
echo "  AI high-performance: 360 MHz"
echo "  DET -> REC mutex   : present"
echo "  AI boost routing   : Vision hook -> Application Logic -> System Adapter -> CPU power"
echo "  DET/REC pipeline   : Face Recognition Service"

# ---------------------------------------------------------------------------
# ESP-IDF 5.5.4 activation
#
# This installation uses:
#   ~/.espressif/tools/activate_idf_v5.5.4.sh
#
# Do NOT assume ~/esp/esp-idf-v5.5.4/export.sh exists.
# ---------------------------------------------------------------------------
ensure_idf() {
    # If the current shell is already activated, keep it.
    if [[ -n "${IDF_PATH:-}" && -f "${IDF_PATH}/tools/idf.py" ]]; then
        return 0
    fi

    if command -v idf.py >/dev/null 2>&1; then
        return 0
    fi

    local candidates=(
        "${IDF_ACTIVATE:-}"
        "$HOME/.espressif/tools/activate_idf_v5.5.4.sh"
        "$HOME/.espressif/v5.5.4/esp-idf/export.sh"
        "$HOME/esp/esp-idf-v5.5.4/export.sh"
    )

    local activation=""
    for candidate in "${candidates[@]}"; do
        if [[ -n "$candidate" && -f "$candidate" ]]; then
            activation="$candidate"
            break
        fi
    done

    if [[ -z "$activation" ]]; then
        echo "[ERROR] ESP-IDF 5.5.4 activation script was not found."
        echo
        echo "Checked:"
        printf '  %s\n' \
            "$HOME/.espressif/tools/activate_idf_v5.5.4.sh" \
            "$HOME/.espressif/v5.5.4/esp-idf/export.sh" \
            "$HOME/esp/esp-idf-v5.5.4/export.sh"
        echo
        echo "You can override it with:"
        echo "  IDF_ACTIVATE=/path/to/activate_or_export.sh $0 $MODE"
        exit 1
    fi

    echo
    echo "[INFO] Activating ESP-IDF with:"
    echo "  $activation"

    # shellcheck disable=SC1090
    source "$activation"

    if [[ -z "${IDF_PATH:-}" || ! -f "${IDF_PATH}/tools/idf.py" ]]; then
        if ! command -v idf.py >/dev/null 2>&1; then
            echo "[ERROR] ESP-IDF activation completed, but idf.py is still unavailable."
            exit 1
        fi
    fi
}

ensure_idf

# Prefer the interpreter from the activated ESP-IDF environment and call idf.py
# by its absolute path. This is more reliable than depending on shell PATH.
if [[ -n "${IDF_PATH:-}" && -f "${IDF_PATH}/tools/idf.py" ]]; then
    PYTHON_BIN="$(command -v python || command -v python3)"
    IDF_CMD=("$PYTHON_BIN" "$IDF_PATH/tools/idf.py")
else
    IDF_CMD=("idf.py")
fi

echo
echo "========== ESP-IDF ENVIRONMENT =========="
echo "  IDF_PATH : ${IDF_PATH:-<not set>}"
echo "  Python   : ${PYTHON_BIN:-PATH idf.py}"
echo "  idf.py   : ${IDF_PATH:+$IDF_PATH/tools/idf.py}"

# Auto-detect a serial port when one was not supplied.
if [[ -z "$PORT" ]]; then
    PORT="$(find /dev -maxdepth 1 \( -name 'ttyACM*' -o -name 'ttyUSB*' \) \
        -print 2>/dev/null | sort | head -n1 || true)"
fi

if [[ -z "$PORT" ]]; then
    echo "[ERROR] No serial device found under /dev/ttyACM* or /dev/ttyUSB*."
    echo "        Connect/reconnect the ESP32-P4 and run:"
    echo "        ls -l /dev/ttyACM* /dev/ttyUSB* 2>/dev/null"
    exit 1
fi

if [[ ! -e "$PORT" ]]; then
    echo "[ERROR] Flash port does not exist: $PORT"
    exit 1
fi

cd "$IDF_PROJECT"

# For the true NN case, refuse to silently benchmark an unoptimized build.
if [[ "$CANONICAL" == "tflm-int8" ]]; then
    if [[ ! -f sdkconfig ]] || ! grep -q '^CONFIG_NN_OPTIMIZED=y' sdkconfig; then
        echo "[ERROR] CONFIG_NN_OPTIMIZED=y is required for the nn backend."
        echo "        Current project: $IDF_PROJECT/sdkconfig"
        exit 1
    fi
fi

echo
echo "============================================================"
echo " ESP32-P4 AI build / flash"
echo "============================================================"
echo "Backend : $LABEL"
echo "Port    : $PORT"
echo "Project : $IDF_PROJECT"
echo "============================================================"

"${IDF_CMD[@]}" reconfigure
"${IDF_CMD[@]}" build

if [[ "$CANONICAL" == "tflm-int8" ]]; then
    map_file="$(find build -maxdepth 1 -type f -name '*.map' -print -quit)"
    if [[ -z "$map_file" ]]; then
        echo "[ERROR] Build map not found; cannot verify ESP-NN linkage."
        exit 1
    fi

    if ! grep -q 'esp-nn' "$map_file"; then
        echo "[ERROR] INT8 build completed but esp-nn was not found in:"
        echo "  $map_file"
        exit 1
    fi

    echo
    echo "[OK] ESP-NN linkage found in $(basename "$map_file")"
fi

"${IDF_CMD[@]}" -p "$PORT" flash

if [[ "${NO_MONITOR:-0}" == "1" ]]; then
    echo
    echo "[OK] Flash complete. Monitor skipped because NO_MONITOR=1."
    exit 0
fi

echo
echo "Starting monitor. Press Ctrl+] to exit."
exec "${IDF_CMD[@]}" -p "$PORT" monitor
