...
typedef struct {
    bool active;
    uint8_t mipi_sc_ctrl;   /* 0x3018 */
    uint8_t mipi_ctrl00;    /* 0x4800 */
    uint8_t mipi_ctrl05;    /* 0x4805 */
} ov5647_light_sleep_state_t;

static ov5647_light_sleep_state_t s_ov5647_light_sleep_state = {0};
...
{
    s_ov5647_light_sleep_state.active = false;
...
static esp_err_t ov5647_attach_power_device(
    i2c_master_dev_handle_t *device)
{
    if (device == NULL || s_camera_i2c_bus == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    ...
    *device = NULL;
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(
            CAMERA_POWER_TAG,
            "CAMERA-LS: OV5647 probe failed: %s",
            esp_err_to_name(ret));
        return ret;
    }
...
esp_err_t app_video_prepare_sensor_for_light_sleep(void)
{
    if (s_ov5647_light_sleep_state.active) {
        return ESP_OK;
    }

    i2c_master_dev_handle_t device = NULL;
    esp_err_t ret = ov5647_attach_power_device(&device);

    if (ret != ESP_OK) {
        return ret;
    }

    ov5647_light_sleep_state_t saved = {0};

    /*
     * STREAMOFF has already executed before this function is called.
     * Save every additional register that the normal ESP-Video
     * STREAMON path does not restore.
     */
    ret = ov5647_read_register(
        device,
        OV5647_REG_MIPI_SC_CTRL,
    ...
    ESP_LOGI(
        CAMERA_POWER_TAG,
        "CAMERA-LS: saved OV5647 registers: "
        "3018=0x%02X 4800=0x%02X 4805=0x%02X",
        saved.mipi_sc_ctrl,
        saved.mipi_ctrl00,
        saved.mipi_ctrl05);

    /*
     * Disable the two MIPI data lanes.
     */
    ret = ov5647_update_register_bits(
        device,
        OV5647_REG_MIPI_CTRL05,
    ...
    /*
     * Gate the clock lane and place it in low-power mode.
     */
    ret = ov5647_update_register_bits(
        device,
        OV5647_REG_MIPI_CTRL00,
    ...
    /*
     * Power down the MIPI transmitter/receiver and suspend
     * the sensor-side MIPI subsystem.
     */
    ret = ov5647_update_register_bits(
        device,
        OV5647_REG_MIPI_SC_CTRL,
    ...
    saved.active = true;
    s_ov5647_light_sleep_state = saved;

    ESP_LOGI(
        CAMERA_POWER_TAG,
        "CAMERA-LS: OV5647 reversible Light-sleep standby ACTIVE");

    goto cleanup;

rollback:
    /*
     * A partial transition is unsafe for a later STREAMON.
     * Restore the exact pre-Light-sleep values before reporting failure.
     */
    ESP_LOGW(
        CAMERA_POWER_TAG,
        "CAMERA-LS: standby failed; restoring OV5647 registers");

    (void)ov5647_write_register(
        device,
        OV5647_REG_MIPI_SC_CTRL,
        saved.mipi_sc_ctrl);
...
cleanup:
    {
        esp_err_t remove_ret = i2c_master_bus_rm_device(device);

        if (remove_ret != ESP_OK && ret == ESP_OK) {
            ret = remove_ret;
        }
    }
...
esp_err_t app_video_restore_sensor_after_light_sleep(void)
{
    if (!s_ov5647_light_sleep_state.active) {
        return ESP_OK;
    }
    ...
    /*
     * Restore sensor-side MIPI power first, then data lanes,
     * then the original MIPI clock/control state.
     *
     * 0x0100 remains in STREAMOFF here. The normal
     * video_stream_start() call will subsequently perform STREAMON.
     */
    ret = ov5647_write_register(
        device,
        OV5647_REG_MIPI_SC_CTRL,
        s_ov5647_light_sleep_state.mipi_sc_ctrl);

    if (ret == ESP_OK) {
        ret = ov5647_write_register(
            device,
            OV5647_REG_MIPI_CTRL05,
            s_ov5647_light_sleep_state.mipi_ctrl05);
    }
    ...
    if (ret == ESP_OK) {
        ret = ov5647_write_register(
            device,
            OV5647_REG_MIPI_CTRL00,
            s_ov5647_light_sleep_state.mipi_ctrl00);
    }
    ...
    if (ret == ESP_OK) {
        ESP_LOGI(
            CAMERA_POWER_TAG,
            "CAMERA-LS: OV5647 MIPI state restored before STREAMON");
