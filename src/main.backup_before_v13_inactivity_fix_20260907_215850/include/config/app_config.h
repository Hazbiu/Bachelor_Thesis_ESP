...
/*
 * IP101GRI Light-sleep network-standby experiment:
 *
 * Do NOT assert the PHY RESET pin in Light-sleep. Keep the PHY powered and use
 * the documented WOL+ slave low-power state instead. The PHY is manually put
 * into WOL+ sleep, where the IP101G down-speeds its sleeping link to 10 Mbps.
 * The P4 manually restores normal PHY operation on application wake.
 *
 * Deep-sleep remains unchanged and still asserts GPIO51 RESET LOW.
 */
#define APP_LIGHT_SLEEP_ETHERNET_WOL_STANDBY         1
