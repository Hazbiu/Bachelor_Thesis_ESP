 ...
 * ----------------------------
 *
 * The FP32 detector compares a real-valued probability against
 * APP_TFLM_BLAZEFACE_SCORE_THRESHOLD (0.75).
 *
 ...
 *     zero_point = 122
 *
 * the desired 0.75 probability corresponds to a logit of about 1.0986,
 ...
 *
 * Correct quantized-domain behavior is to quantize the requested threshold
 * itself with nearest rounding, exactly as a real value is represented on
 * the tensor's quantization grid, then compare integer logits directly.
 * For this model that selects q>=123, the nearest representable threshold.
 *
...
static int quantized_score_threshold_q(const TfLiteTensor *scores)
{
    ...
    const float probability = std::max(
        1.0e-6f,
        std::min(
            APP_TFLM_BLAZEFACE_SCORE_THRESHOLD,
            1.0f - 1.0e-6f));
...
{
    if (!scores || scores->params.scale <= 0.0f) {
        return APP_TFLM_BLAZEFACE_SCORE_THRESHOLD;
    }
    ...
    bool rgb565)
{
    ...
    ESP_LOGI(
        ...
        (double)max_score,
        max_score_q,
        (double)APP_TFLM_BLAZEFACE_SCORE_THRESHOLD,
