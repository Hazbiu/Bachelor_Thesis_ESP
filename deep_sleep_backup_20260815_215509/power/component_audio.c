...
esp_err_t component_audio_disable_for_deep_sleep(void)
{
    ...
    const int level_after_hold = gpio_get_level(AUDIO_POWER_AMP_GPIO);

    if (level_after_hold != AUDIO_AMP_DISABLED) {
        ESP_LOGE(
            TAG,
            "Audio amplifier EN reads %d after hold (expected %d)",
            level_after_hold,
            AUDIO_AMP_DISABLED);
    ...
    ESP_LOGI(
        TAG,
        "Audio amplifier disabled: GPIO%d held LOW (read-back=%d)",
        AUDIO_POWER_AMP_GPIO,
        level_after_hold);
