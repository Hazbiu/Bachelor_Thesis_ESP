...
esp_err_t component_sdcard_disable_for_deep_sleep(void)
{
    ...
    const int level_after_hold = gpio_get_level(SDCARD_POWER_GPIO);

    if (level_after_hold != SDCARD_POWER_OFF_LEVEL) {
        ESP_LOGE(
            TAG,
            "microSD gate reads %d after hold (expected %d); SD1_VDD is still "
            "connected",
            level_after_hold,
            SDCARD_POWER_OFF_LEVEL);
    ...
    ESP_LOGI(
        TAG,
        "microSD power disabled: GPIO%d held HIGH (read-back=%d)",
        SDCARD_POWER_GPIO,
        level_after_hold);
