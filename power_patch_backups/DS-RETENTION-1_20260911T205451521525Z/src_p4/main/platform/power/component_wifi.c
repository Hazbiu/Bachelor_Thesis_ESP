 ...
 * disabled the C6.
 */
static bool s_c6_disabled = true;
...
static esp_err_t boot_c6_for_active_policy(void)
{
    /*
     * A cold C6 boot with GPIO2 LOW is reserved for the real P4 Deep-sleep
     * boundary. Therefore a normal ACTIVE boot first presents GPIO2 HIGH,
     * resets/releases CHIP_PU, lets the companion start at 80 MHz, and then
     * drops GPIO2 LOW so the already-running companion selects 160 MHz.
     *
     * This reset sequence is used only when the C6 is already OFF/reset (for
     * example at application boot or after a real Deep-sleep wake). It is NOT
     * used when entering or leaving P4 Light-sleep.
     */
    ...
    ESP_LOGI(
        TAG,
        "ESP32-C6 ACTIVE boot complete: CHIP_PU=HIGH mode_gpio=LOW requested_cpu=160MHz");
    return ESP_OK;
}
...
{
    /*
     * User requirement: P4 Light-sleep must NOT reset or power-cycle the C6.
     * The already-running companion sees GPIO2 HIGH and lowers its CPU policy
     * from 160 MHz to 80 MHz in place while CHIP_PU remains continuously HIGH.
     */
    if (!s_user_enabled) {
        ESP_LOGI(
            TAG,
            "ESP32-C6 remains OFF in P4 Light-sleep because saved Wi-Fi policy is OFF");
        return ESP_OK;
    }

    if (s_c6_disabled) {
        /*
         * Defensive recovery only: the saved policy says ON but the software
         * state says the C6 is already OFF. Bring it up before requesting the
         * retained Light-sleep frequency. This does not switch off a running C6.
         */
        esp_err_t ret = boot_c6_for_active_policy();
        if (ret != ESP_OK) {
            return ret;
        }
    }
    ...
    /* CHIP_PU is intentionally untouched here. */
    vTaskDelay(pdMS_TO_TICKS(APP_PWR_WIFI_C6_LIGHT_BOOT_SETTLE_MS));

    if (gpio_get_level(WIFI_C6_CHIP_PU_GPIO) != WIFI_C6_ENABLED_LEVEL) {
        ESP_LOGE(
            TAG,
            "C6 Light-sleep request rejected: CHIP_PU is not continuously HIGH");
        return ESP_ERR_INVALID_STATE;
    }

    ESP_LOGI(
        TAG,
        "ESP32-C6 retained for P4 Light-sleep WITHOUT RESET: "
        "CHIP_PU=HIGH mode_gpio=HIGH requested_cpu=80MHz");
    return ESP_OK;
}
...
{
    /*
     * The running C6 sees GPIO2 HIGH -> LOW and restores 160 MHz. CHIP_PU is
     * never pulsed LOW during this Light-sleep wake path.
     */
    esp_err_t ret = configure_c6_mode_pin(
        WIFI_C6_MODE_DEEP_LEVEL,
        false);
    ...
    if (s_user_enabled) {
        if (s_c6_disabled) {
            /* Unexpected state: recover ACTIVE operation after the P4 woke. */
            return boot_c6_for_active_policy();
        }

        vTaskDelay(pdMS_TO_TICKS(APP_PWR_WIFI_C6_ACTIVE_SETTLE_MS));
        ESP_LOGI(
            TAG,
            "ESP32-C6 restored after P4 Light-sleep WITHOUT RESET: "
            "CHIP_PU=HIGH mode_gpio=LOW requested_cpu=160MHz");
        return ESP_OK;
    }

    ESP_LOGI(
        TAG,
        "Saved Wi-Fi policy is OFF; ESP32-C6 remains held in reset after Light-sleep");
...
{
    if (s_user_enabled) {
        ESP_LOGI(TAG, "Restoring ESP32-C6 after failed Deep-sleep");
        return boot_c6_for_active_policy();
    }
...
esp_err_t component_wifi_set_enabled(bool enabled)
{
    ...
    if (enabled) {
        if (s_c6_disabled) {
            ret = boot_c6_for_active_policy();
        } else {
            /*
             * Idempotent re-application after a Light-sleep wake: only request
             * ACTIVE frequency and keep CHIP_PU HIGH. Do not reset the C6.
             */
            ret = configure_c6_mode_pin(
                WIFI_C6_MODE_DEEP_LEVEL,
                false);
