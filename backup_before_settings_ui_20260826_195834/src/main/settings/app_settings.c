...
esp_err_t app_settings_init(void)
{
    ...
    if (ret == ESP_ERR_NVS_NOT_FOUND) {
        ...
        ESP_LOGI(TAG, "No saved settings; using light mode with network enabled");
        return ESP_OK;
    }
    if (ret != ESP_OK) {
    ...
    ESP_LOGI(
        TAG,
        "Loaded settings: theme=%s ethernet=%s wifi=%s",
    ...
    app_settings_authorized_users_t *users_out)
{
    ...
    if (bsp_sdcard == NULL) {
        ...
        if (mount_ret != ESP_OK) {
            ESP_LOGE(TAG, "Could not mount microSD for user refresh: %s",
                     esp_err_to_name(mount_ret));
            return mount_ret;
        }
        ESP_LOGI(TAG, "microSD mounted for authorized-user refresh");
    }
    ...
    while ((entry = readdir(directory)) != NULL &&
        ...
        snprintf(
            users_out->names[users_out->count],
