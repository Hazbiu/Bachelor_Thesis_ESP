...
esp_err_t TflmModelRunner::init(const char *model_path, size_t tensor_arena_bytes)
{
    ...
    /*
     * TFLM works best with a 16-byte aligned arena. Keep the raw allocation so
     * it can be freed while exposing an aligned interior pointer.
     */
    ...
    resolver_ = new (std::nothrow) tflite::AllOpsResolver();
    if (!resolver_) {
        return ESP_ERR_NO_MEM;
    }
    ...
    if (interpreter_->AllocateTensors() != kTfLiteOk) {
        ESP_LOGE(
            TAG,
            "AllocateTensors failed for %s; increase arena or check model ops",
            model_path);
        return ESP_FAIL;
    }
