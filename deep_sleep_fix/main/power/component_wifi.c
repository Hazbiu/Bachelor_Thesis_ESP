...
esp_err_t component_wifi_disable_for_deep_sleep(void)
{
    ...
    /*
     * Read the pad back through the still-enabled input path. If it never
     * settles LOW, the external pull-up has won and the ESP32-C6 is running,
     * which is worth tens of milliamps during "Deep-sleep".
     */
    ...
    if (level_after_hold != WIFI_C6_DISABLED_LEVEL) {
        ESP_LOGE(
            TAG,
            "ESP32-C6 CHIP_PU still reads %d after holding for %" PRIu32
            " ms (expected %d); the coprocessor is still powered",
            level_after_hold,
            settle_ms,
            WIFI_C6_DISABLED_LEVEL);
