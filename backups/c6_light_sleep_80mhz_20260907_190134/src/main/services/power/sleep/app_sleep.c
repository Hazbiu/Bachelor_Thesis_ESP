...
static esp_err_t suspend_aux_peripherals_for_light_sleep(void)
{
    ...
    /*
     * Do not hold the C6 in reset in P4 Light-sleep. GPIO6->C6 GPIO2 is set
     * HIGH and the C6 is cleanly rebooted into its dedicated 80 MHz retained
     * companion mode. Deep-sleep keeps the existing self-Deep-sleep path.
     */
    ...
    if (first_error == ESP_OK) {
        ...
        ESP_LOGI(
            ...
            "c6=RETAINED_80MHZ chip_pu=HIGH mode_gpio=HIGH gt911=POLLING",
            audio_state,
            sdcard_state,
            ethernet_state);
    }
